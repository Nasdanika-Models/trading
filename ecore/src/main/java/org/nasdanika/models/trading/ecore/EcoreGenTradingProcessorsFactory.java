...
@EPackageNodeProcessorFactory(nsURI = TradingPackage.eNS_URI)
public class EcoreGenTradingProcessorsFactory {

	private Context context;
	
	// TODO - other classes
	
	public EcoreGenTradingProcessorsFactory(Context context) {
		this.context = context;
	}
		
	@EPackageNodeProcessorFactory(
			label = "Trading Model",
			description = "Trading model based on Ecore",
			actionPrototype = """
	                app-action:
	                  content:
	                    content-markdown:
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					Files.writeString(new File("target/trading.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
